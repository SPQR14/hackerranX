...
def calcular_imc(altura = 0, peso = 0):
...
print(calcular_imc(alturas, pesos))
...
for i in range(1000000):
    calcular_imc(alturas, pesos)
t1 = perf()
print(f'Tiempo total de ejecución usando funciones universales de numpy después de {i} veces: {round(t1 - t0, 2)}')


# In[24]:


# Comparando con listas
alturas = [1.74, 1.8, 1.78, 1.68, 1.78, 1.7, 1.74, 1.74, 1.73, 1.79,
           1.78, 1.72, 1.65, 1.78, 1.7, 1.68, 1.79, 1.68, 1.82, 1.72,
           1.75, 1.66, 1.67, 1.79, 1.75, 1.65, 1.78, 1.73, 1.71, 1.81,
           1.73, 1.71, 1.68, 1.74, 1.75, 1.68, 1.74, 1.81, 1.73, 1.82,
           1.8, 1.67, 1.73, 1.7, 1.73, 1.7, 1.81, 1.81, 1.76, 1.82, 1.68,
...
pesos = [81.4, 88.7, 87.3, 62.7, 81.6, 80.9, 74.6, 84.7, 76.7, 88.3,
         84.6, 74, 57.7, 84.1, 79.7, 63.8, 88.4, 71.2, 87.1, 67, 80.7,
         74.7, 60.5, 85.9, 72.4, 59.7, 87.3, 85.7, 64.1, 91.9, 82.8, 75.7,
         60.2, 73.1, 74.7, 65.9, 80.9, 91.3, 76, 86.8, 80.7, 74.2, 83.1,
         77.8, 84.5, 58.8, 89.5, 87, 84, 89.9, 56.5, 80.2, 61.8, 86.3,
...
def calcular_imc(alturas, pesos):
...
for i in range(1000000):
    for j in range(len(pesos)):
        imc.append(calcular_imc(pesos[j], alturas[j]))
t1 = perf()
print(f'Tiempo total de ejecución usando listas después de {i} veces: {round(t1 - t0, 2)}')
